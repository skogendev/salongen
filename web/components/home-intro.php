<div class="section bg-krem-light py-0" style="padding: 0;">
  <div class="h-full overflow-hidden absolute w-full">
    <div class="absolute container w-full pin-l u-flexgrid-g" style="min-height: 90vh">
      <div class="col-2">
        <div class="overflow-hidden relative" style="transform: scale(2); transform-origin: top right; position: relative; top: 10vh;">
          <div class="sticky-image">
            <img data-src="/assets/img/tmp19.jpg" class="inview lazyload-img lazyload">
          </div>
        </div>
      </div>
      <div class="col-5 col-off-5">
        <div class="mt-12 sticky-image">
          <img src="/assets/img/tmp21.jpg" class="inview mt-32 lazyload-img lazyload">
          <div class="text-sm mt-4">
            <p><span class="font-serif text-eple">FIG 2</span><br>
            Få den beste maten du kan tenke deg</p>
          </div>
        </div>
      </div>
      <div class="col-3 col-off-2 -mt-32 sticky-image">
        <img src="/assets/img/tmp20.jpg" class="inview mt-8 lazyload-img lazyload">
        <div class="text-sm mt-4">
          <p><span class="font-serif text-eple">FIG 1</span><br>
            Vi skreddersyr alle arrangement</p>
        </div>
      </div>
      <div class="col-2 col-off-8 mt-16">
        <div class="sticky-image">
          <div style="transform: scale(1.5); transform-origin: bottom left;">
            <img src="/assets/img/tmp1.jpg" class="inview lazyload-img lazyload">
          </div>
        </div>
      </div>
    </div>
  </div>
  <div class="container bg-segl relative">
    <div class="u-flexgrid-g " style="min-height: 90vh">
      <div class="col-12 lg:col-8 xl:col-off-2 xl:col-4 text-left pt-32">
        <h1 class="heading-xl mb-8 js-load-stagger pt-12">
          Det personlige konferansehuset du har ventet på
        </h1>


            <div class="js-load-stagger mt-8">
              <a href="/moterom.php" class="button">Book møterom</a>
              <a href="/konferanse.php" class="button">Book konferanse</a>
            </div>
      </div>
    </div>
  </div>
</div>
