
  <div class="section bg-split">
    <div class="container">
      <div class="u-flexgrid-g text-left">
        <div class="col-10 lg:col-5 xl:col-5">
          <div class="w-4/5">
            <div class="heading-sm text-eple mb-4">Konferanse</div>
            <h2 class="heading-lg">Book ditt neste arrangement hos oss</h2>
            <hr class="hr mx-0">
              <p>Salongen er et personlig og inspirerende konferansehus med et stort hjerte for å skreddersy arrangementer av høy kvalitet. Salongen har møterom og fasiliteter som passer fint for både små møter og større arrangementer. </p>
              <a href="/konferanse.php" class="button mt-4">Les mer om konferanse</a>
          </div>
        </div>
        <div class="col-10 lg:col-5 xl:col-5 xl:col-off-2">
          <div class="w-4/5">
            <div class="heading-sm text-eple mb-4">Møterom</div>
            <h2 class="heading-lg">Det perfekte sted for møter</h2>
            <hr class="hr mx-0">
              <p>Salongen er et personlig og inspirerende konferansehus med et stort hjerte for å skreddersy arrangementer av høy kvalitet. </p>
              <div class="button mt-4">
                <input class="p-0 m-0 c-calendar border-none text-base heading-sm" style="font-size: 1em;" type="text" placeholder="Velg en dato"  readonly="readonly">
              </div>
      				<button class="button -ml-8 button-fill">Sjekk</button>
          </div>
        </div>
      </div>
    </div>
  </div>
